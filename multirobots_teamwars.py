...
'''
    SensorBelt = les capteurs
    [0] = -170   #capteur arriere
    [1] = -80    #capteur gauche
    ...
    rotation : 1 sens montre normal ou -1 sens anti-horaire
...
class AgentTypeA(object):
    ...
    teamname = "Team O Thé" # A modifier avec le nom de votre équipe

    def stepController(self):
         #distance de chaque sensor a l'obstacle:
        dist_170=self.getDistanceAtSensor(0)
        dist_80=self.getDistanceAtSensor(1)
        dist_40=self.getDistanceAtSensor(2)
        dist_20=self.getDistanceAtSensor(3)
        dist20=self.getDistanceAtSensor(4)
        dist40=self.getDistanceAtSensor(5)
        dist80=self.getDistanceAtSensor(6)
        dist170=self.getDistanceAtSensor(7)
        l_dist=[dist_170,dist_80,dist_40,dist_20,dist20,dist40,dist80,dist170]
        #type de l'obstacle a chaque sensor:
        sens_170=self.getObjectTypeAtSensor(0)
        sens_80=self.getObjectTypeAtSensor(1)
        sens_40=self.getObjectTypeAtSensor(2)
        sens_20=self.getObjectTypeAtSensor(3)
        sens20=self.getObjectTypeAtSensor(4)
        sens40=self.getObjectTypeAtSensor(5)
        sens80=self.getObjectTypeAtSensor(6)
        sens170=self.getObjectTypeAtSensor(7)
        l_sens=[sens_170,sens_80,sens_40,sens_20,sens20,sens40,sens80, sens_170]
        p = self.robot
        ...
        color( (253, 63, 146) )
        circle( *self.getRobot().get_centroid() , r = 22) # je dessine un rond rose autour de ce robot
        ...
        if strategy == 0:
            ...
            self.setTranslationValue(1) # normalisé -1,+1
            # Éviter les obstacles (murs et robots)
            sensor_infos = sensors[p]
            for impact in sensor_infos[2:6]:
                if impact.layer == 'joueur':
                    dist = impact.dist_from_border
                    if dist < 5:
                        if SensorBelt[sensor_infos.index(impact)] > 0:
                            p.rotate(-1)
                        else:
                            p.rotate(1)
                else :
            ...
            # Éviter les robots et ignorer les murs
            sensor_infos = sensors[p]
            for impact in sensor_infos[2:6]:
                if impact.layer == 'joueur':
                    dist = impact.dist_from_border
            ...
            # Éviter les murs et ignorer les robots
            sensor_infos = sensors[p]
            for impact in sensor_infos[2:6]:
                dist = impact.dist_from_border
                if dist < 5:
            ...
            # Aller vers les robots et ignorer les murs
            sensor_infos = sensors[p]
            for impact in sensor_infos[2:6]:
                if impact.layer == 'joueur':
                    dist = impact.dist_from_border
            ...
            # Aller vers les murs et ignorer les robots
            sensor_infos = sensors[p]
            for impact in sensor_infos[2:6]:
                dist = impact.dist_from_border
                if dist == 0:
            ...
            distmin=20
            if(sens_40==2 or sens_40==1):
                self.setRotationValue( 0 )
                self.setTranslationValue(1)
                if(dist_40<distmin):
                    self.setRotationValue( 0.10 )
            elif(sens40==2 or sens40==1):
                self.setRotationValue( 0 )
                self.setTranslationValue(1) 
                if(dist40<distmin):
                    self.setRotationValue( -0.10 )
            elif(sens_80==2 or sens_80==1):
                self.setRotationValue( 0 )
                self.setTranslationValue(1)
                if(dist_80<distmin):
                    self.setRotationValue( 0.25 )
            elif(sens80==2  or sens80==1):
                self.setRotationValue( 0 )
                self.setTranslationValue(1)
                if(dist80<distmin):
                    self.setRotationValue( -0.25 )
            elif(sens_20==2  or sens_20==1):
                self.setRotationValue( 0 )
                self.setTranslationValue(1)
                if(dist_20<distmin):
                    self.setRotationValue( 0.40 )
            elif(sens20==2 or sens20==1):
                self.setRotationValue( 0 )
                self.setTranslationValue(1) 
                if(dist20<distmin):
                    self.setRotationValue( -0.40 )
